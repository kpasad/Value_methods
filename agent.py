...
class Agent():
    ...
    def step_priority_rb(self, state, action, reward, next_state, done):

        beta=1
        self.priority_replay.push(state, action, reward, next_state, done)
        # Learn every UPDATE_EVERY time steps.
        self.t_step = (self.t_step + 1) % UPDATE_EVERY
        if self.t_step == 0:
            # If enough samples are available in memory, get random subset and learn
            if len(self.priority_replay.buffer) > BATCH_SIZE:
                experiences= self.priority_replay.sample(BATCH_SIZE,beta)
                self.learn_priority_rp(experiences, GAMMA)
    ...
    def learn_priority_rp(self, experiences, gamma):
        ...
        loss = (Q_expected-Q_targets).pow(2) * weights.unsqueeze(1)
        prios = loss+1e-5
        loss =loss.mean()
        self.optimizer.zero_grad()
        loss.backward()
        self.priority_replay.update_priorities(indices,prios.data.cpu().numpy())
        self.optimizer.step()
